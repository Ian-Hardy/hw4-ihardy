#######################################
# Code largely inspired/based off of the work by David Callanan 
# in his youtube series "Make YOUR OWN Programming Language"
# Code I forked from can be found here:
...
class Context:
	...
	def __repr__(self):
		keys = list(self.symbol_table.symbols.keys())
		keys.sort()
		out_str = '{'
		for i, val in enumerate(keys):
			if i>0:
				out_str += ', '
			if isinstance(self.symbol_table.symbols[val], List):
				list_str = '['
				for j, element in enumerate(self.symbol_table.symbols[val].elements):
					if j>0:
						list_str += ', '
				...
				out_str += str(val)+' → '+str(self.symbol_table.symbols[val])
		out_str += '}'
		return out_str
